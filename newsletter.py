...
Original file is located at
    https://colab.research.google.com/drive/1xgO3yJ9SlvnGIT_mhuvhkJzE7UogT-DP
...
# ============================
# 2. 날짜 자동 설정 (KST 전날 08:00 ~ 당일 08:00)
# ============================
...
# 기준 종료 시각: "오늘 08:00 KST"
end_kst = now_kst.replace(hour=8, minute=0, second=0, microsecond=0)

# 지금이 08:00 이전이면, 종료 시각을 "어제 08:00"으로 조정
if now_kst < end_kst:
    end_kst -= timedelta(days=1)

# 시작 시각: 종료 시각의 전날 08:00
start_kst = end_kst - timedelta(days=1)

# UTC 변환 (API는 UTC 필요)
date_from_utc = start_kst.astimezone(timezone.utc)
date_to_utc   = end_kst.astimezone(timezone.utc)

# ✅ ISO8601(datetime)로 저장 (주의: 'Z'는 fromisoformat 파싱 이슈가 있어 +00:00 형태 유지)
DATE_FROM = date_from_utc.isoformat(timespec="seconds")  # 예: 2025-12-14T23:00:00+00:00
DATE_TO   = date_to_utc.isoformat(timespec="seconds")    # 예: 2025-12-15T23:00:00+00:00

print("=" * 60)
print(f"🕐 현재 KST 시간: {now_kst.strftime('%Y-%m-%d %H:%M:%S')}")
print(f"📅 검색 범위 (KST): {start_kst.strftime('%Y-%m-%d %H:%M:%S')} ~ {end_kst.strftime('%Y-%m-%d %H:%M:%S')}")
...
TOPIC_MORE_FILENAMES = {
    1: "more_geoint_test.html",        # GeoINT 전용 추가 기사
    2: "more_aviation_test.html",      # 항공/드론 전용 추가 기사
    3: "more_ai_platform_test.html",   # AI 데이터 플랫폼 전용 추가 기사
    4: "more_satellite_test.html",     # 위성 영상 전용 추가 기사
}
...
# 연구동향 more 페이지
RESEARCH_MORE_FILENAME = "more_research_test.html"
...
# 한국 시간 기준 오늘 날짜로 폴더 생성 (예: 2025test/11test/26test)
KST = timezone(timedelta(hours=9))
now_kst = datetime.now(KST)

# ✅ GitHub 업로드 경로/파일명 suffix (repo가 뒤죽박죽 되지 않게 테스트용 네임스페이스)
NAME_SUFFIX = "test"

def _add_suffix_before_ext(filename: str, suffix: str = NAME_SUFFIX) -> str:
    """예: archive.html -> archive_test.html"""
    if not suffix:
        return filename
    if "." not in filename:
...
# 폴더명은 사용자가 원한 형태대로: 2025test/12test/16test
YEAR_DIR  = f"{now_kst.year}{NAME_SUFFIX}"
MONTH_DIR = f"{now_kst.month:02d}{NAME_SUFFIX}"
DAY_DIR   = f"{now_kst.day:02d}{NAME_SUFFIX}"

FOLDER_PATH = f"{YEAR_DIR}/{MONTH_DIR}/{DAY_DIR}"

# 파일명도 모두 suffix 적용
INDEX_FILENAME = _add_suffix_before_ext("index.html")
ARCHIVE_HTML_FILENAME = _add_suffix_before_ext("archive.html")
ARCHIVE_JSON_FILENAME = _add_suffix_before_ext("archive.json")

# 메인 페이지 및 토픽별 추가 기사 페이지 URL
MAIN_PAGE_URL = f"{BASE_URL}/{FOLDER_PATH}/{INDEX_FILENAME}"
...
# ▼ 이전 뉴스레터 아카이브 페이지 설정
ARCHIVE_PAGE_PATH = f"docs/{ARCHIVE_HTML_FILENAME}"
ARCHIVE_PAGE_URL = f"{BASE_URL}/{ARCHIVE_HTML_FILENAME}"
...
ARCHIVE_JSON_PATH = f"docs/{ARCHIVE_JSON_FILENAME}"
...
def load_existing_archive():
    ...
    # --------------------------------------------------
    json_text = get_github_file_text(ARCHIVE_JSON_PATH)
    if json_text:
        try:
    ...
    for year_name in year_dirs:
        ...
        month_dirs = [m["name"] for m in month_children if m.get("type") == "dir" and m.get("name","").endswith(NAME_SUFFIX) and m.get("name","")[:-len(NAME_SUFFIX)].isdigit()]

        for month_name in month_dirs:
            month_path = f"docs/{year_name}/{month_name}"
            day_children = list_github_directory(month_path)
            day_dirs = [d["name"] for d in day_children if d.get("type") == "dir" and d.get("name","").endswith(NAME_SUFFIX) and d.get("name","")[:-len(NAME_SUFFIX)].isdigit()]
            ...
            for day_name in day_dirs:
                ...
                try:
                    date_obj = datetime(int(year_name[:-len(NAME_SUFFIX)]), int(month_name[:-len(NAME_SUFFIX)]), int(day_name[:-len(NAME_SUFFIX)])).date()
                except ValueError:
                    continue
                ...
                url = f"{BASE_URL}/{year_name}/{month_name}/{day_name}/{INDEX_FILENAME}"
...
# 파일 저장
with open("newsletter_test.html", "w", encoding="utf-8") as f:
...
with open(RESEARCH_MORE_FILENAME, "w", encoding="utf-8") as f:
    f.write(research_more_html)

print(f"{RESEARCH_MORE_FILENAME} 저장 완료")
...
main_repo_path = f"docs/{FOLDER_PATH}/{INDEX_FILENAME}"
...
# 로컬 저장 (HTML)
with open(ARCHIVE_HTML_FILENAME, "w", encoding="utf-8") as f:
...
# 로컬 저장 (JSON)
with open(ARCHIVE_JSON_FILENAME, "w", encoding="utf-8") as f:
...
if SEND_EMAIL and TO_EMAIL:
    ...
    with open("newsletter_test.html", "r", encoding="utf-8") as f:
        html_content = f.read()
