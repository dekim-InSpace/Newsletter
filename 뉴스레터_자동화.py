...
# 1. 라이브러리 설치
# ============================
!pip install -q requests pandas python-dateutil openai beautifulsoup4 feedparser
    ...
    for _, row in topic_df.iterrows():
        ...
        for kept in dedup_rows:
            ...
            if cur_title and kept_title and cur_title == kept_title:
                is_dup = True
                break
            if is_near_duplicate(cur_text, kept_text, threshold=0.9):
                is_dup = True
                break
...
# 9. (선택) 이메일로 바로 보내기
# ============================
SEND_EMAIL = True  # 이메일 자동 전송을 켜고 싶으면 True 로 변경
...
if SEND_EMAIL:
    ...
    print("SEND_EMAIL=False 상태이므로, 메일은 보내지 않았습니다.")

# ============================
# 🚀 GitHub에 자동 업로드 (Colab 전용)
# ============================
AUTO_PUSH_TO_GITHUB = True

if IN_COLAB and AUTO_PUSH_TO_GITHUB:
    import os
    from google.colab import userdata

    print("\n" + "="*60)
    print("🔗 GitHub 저장소에 코드 업로드 중...")
    print("="*60)
    ...
    !git config user.name "junwoo092041"
    !git config user.email "junwoo092041@inspace.re.kr"
    !git pull origin main

    import shutil
    notebook_name = "뉴스레터_자동화.py"
    shutil.copy('/content/뉴스레터_자동화__49_.py', f'./{notebook_name}')

    !git add {notebook_name}
    !git commit -m "Update newsletter from Colab"
